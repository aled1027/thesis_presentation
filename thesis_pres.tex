...
\tikzstyle{mynode} = [rectangle, minimum width=2cm, minimum height=1cm,text centered, draw=black]
\tikzstyle{blanknode} = [rectangle, maximum width=0cm, minimum height=0cm, draw=white]
...
\renewcommand<>{\item}[1]{\only#2{\beameroriginal{\item}{#1}}}
    ...
    \begin{frame}
        ...
        \begin{frame}
            \frametitle{Introducing Online/Offline}
            \begin{itemize}
                \item Imagine two banks use secure computation during their daily operations
                \item At night - the offline phase - they exchanged garbled circuits (the garbled tables)
                \item During the day - the online phase - they exchnage input wire labels and use the pre-exchanged garbled circuits.
                \item This is better for the banks, as the computation performs faster and isn't a burden on them.
                \item Pitfalls
                    \begin{itemize}
                        \item Functions are decided at night - no room for flexibility
                        \item Input size is fixed at night 
                            \begin{itemize}
                                \item rectifiable if the input is padded, but that's inefficient and annoying
                            \end{itemize}
                    \end{itemize}
            \end{itemize}
        \end{frame}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                \item Suppose that we are chaining a garbled circuit with output wire $A$ to garbled circuit with input wire $B$.
                \item We want $A_0 \to B_0$ and $A_0 \oplus \Delta \to B_0 \oplus \Delta$.
                \item Straightforward:
                    \begin{itemize}
                        \item Alice sends Bob $\Gamma = A_0 \oplus B_0$
                        \item Bob sets $B_*$ to  $A_* \oplus \Gamma$.
                        \item TODO haven't introduced * star notation.
                    \end{itemize}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                \item In online phase, communicate a ciphertext per chain
                \item This can be a lot: 
                    \begin{itemize}
                        \item Suppose $100$ by $100$ matrix with entries of max value $2^8$
                        \item Then requires $256,000$ ciphertexts.
                        \item This can be reduced to a single ciphertext.
                    \end{itemize}
            \end{itemize}
        \end{frame}
        ...
        \begin{frame}
            ...
            \begin{itemize}
                ...
                \item TODO SHOW EXAMPLE OF CONSECUTIVE WIRES
                \item Solution: give consecutive wire labels a predictable pattern while maintaining the Free XOR condition
                \item Set the $i$th wire label to $A \oplus H (T \oplus (i || b))$
                    \begin{itemize}
                        \item Where $H$ is a hash function
                        \item TODO figure out what to do about hash function
                    \end{itemize}
            \end{itemize}
        \end{frame}
